...
def callback_work(call):
    ...
    cursor.execute(
        ...
        id INTEGER PRIMARY KEY, us TEXT, ph VARCHAR(30), pr TEXT)''')
    conn.commit()
    if call.data == 'mani':
        bot.send_message(call.message.chat.id,
                         user + ' Вы записаны на маникюр. В ближайшее время с вами свяжется специалист.')
        cursor.execute('''INSERT INTO book(us, ph, pr) VALUES(?, ?, 'маникюр')''', (user, phone,))
        conn.commit()
    elif call.data == 'ped':
        bot.send_message(call.message.chat.id,
                         user + ' Вы записаны на педикюр. В ближайшее время с вами свяжется специалист.')
